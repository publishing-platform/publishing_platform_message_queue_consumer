...
RSpec.describe PublishingPlatformMessageQueueConsumer::Consumer do
  ...
  describe "#run" do
    ...
    it "calls the heartbeat processor when subscribing to messages" do
      ...
      heartbeat_processor_stub = instance_double(PublishingPlatformMessageQueueConsumer::HeartbeatProcessor)
      allow(PublishingPlatformMessageQueueConsumer::HeartbeatProcessor).to receive(:new).and_return(heartbeat_processor_stub)

      expect(heartbeat_processor_stub).to receive(:process).with(kind_of(PublishingPlatformMessageQueueConsumer::Message))
    ...
    context "when a SignalException is raised" do
      ...
      context "and the signal is an unexpected one" do
        ...
        it "gracefully exits after notifying PublishingPlatformError" do
          expect(PublishingPlatformError).to receive(:notify).with(error)
          expect { described_class.new(queue_name: "some-queue", processor: client_processor, rabbitmq_connection: stubs.connection, logger:).run }.to raise_error(SystemExit)
        end
      end
    end
